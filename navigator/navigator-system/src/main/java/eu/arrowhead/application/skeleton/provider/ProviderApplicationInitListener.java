...
public class ProviderApplicationInitListener extends ApplicationInitListener {
	...
	protected void customInit(final ContextRefreshedEvent event) {
		...
		final ServiceRegistryRequestDTO startServiceRequest = createServiceRegistryRequest(NavigatorSystemConstants.START_SERVICE_DEFINITION, NavigatorSystemConstants.START_URI, HttpMethod.GET);		
		arrowheadService.forceRegisterServiceToServiceRegistry(startServiceRequest);
	}
	...
	public void customDestroy() {
		...
		arrowheadService.unregisterServiceFromServiceRegistry(NavigatorSystemConstants.START_SERVICE_DEFINITION, NavigatorSystemConstants.START_URI);
	}
	...
	private ServiceRegistryRequestDTO createServiceRegistryRequest(final String serviceDefinition, final String serviceUri, final HttpMethod httpMethod) {
		...
		if (sslEnabled && tokenSecurityFilterEnabled) {
			...
			serviceRegistryRequest.setInterfaces(List.of(NavigatorSystemConstants.INTERFACE_SECURE));
		} else if (sslEnabled) {
			systemRequest.setAuthenticationInfo(Base64.getEncoder().encodeToString(arrowheadService.getMyPublicKey().getEncoded()));
			...
			serviceRegistryRequest.setInterfaces(List.of(NavigatorSystemConstants.INTERFACE_INSECURE));
		}
		serviceRegistryRequest.setProviderSystem(systemRequest);
		serviceRegistryRequest.setServiceUri(serviceUri);
		serviceRegistryRequest.setMetadata(new HashMap<>());
		serviceRegistryRequest.getMetadata().put(NavigatorSystemConstants.HTTP_METHOD, httpMethod.name());
